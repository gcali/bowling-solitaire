...
.pin-table
    ...
    .pin-row(v-for="cardRow in pinRows" :key="cardRow.id")
        CardComponent(
            v-for="card in keyUp(cardRow.element)" 
            :card="card.element" 
            :key="card.id" 
            @click="pinClick(card.element)"
            :forceHighlight="(highlightCentral && card.element.isCentral) || (highlightBack && card.element.isBackRow)"
        ).pin-card
    .action-area
...
export default class PinTable extends Vue {
    ...
    @Prop({ default: false })
    private highlightCentral!: boolean;

    @Prop({ default: false })
    private highlightBack!: boolean;
